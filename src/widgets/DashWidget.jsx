 ...
 */
export function DashWidget({ id, tickets, articles, users, currentUser, onOpenTicket, priorityCatalog }) {
  // ── Stat cards ──────────────────────────────────────────────────────────────
  if (id.startsWith("stat_")) {
    return <StatWidget id={id} tickets={tickets} currentUser={currentUser} priorityCatalog={priorityCatalog} />;
  }

  // ── Activity lists ───────────────────────────────────────────────────────────
  if (id === "recent")        return <RecentTickets tickets={tickets} users={users} onOpenTicket={onOpenTicket} priorityCatalog={priorityCatalog} />;
  if (id === "critical_list") return <CriticalList  tickets={tickets} onOpenTicket={onOpenTicket} priorityCatalog={priorityCatalog} />;
  if (id === "sla_risk")      return <SLARisk       tickets={tickets} onOpenTicket={onOpenTicket} priorityCatalog={priorityCatalog} />;
  ...
  if (id === "by_priority") return <ByPriorityChart tickets={tickets} priorityCatalog={priorityCatalog} />;
